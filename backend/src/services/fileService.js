...
function normalizeFolder(folder = "") {
  ...
  if (!/^[a-z0-9_-]+$/i.test(value)) {
    throw new ApiError(400, "Invalid upload folder", "INVALID_UPLOAD_FOLDER");
  }

  return value;
}

async function ensureUploadDir(folder = "") {
  const safeFolder = normalizeFolder(folder);
  const directory = safeFolder
    ? path.join(env.uploadDir, safeFolder)
    : env.uploadDir;
...
async function moveFile(source, destination) {
  try {
    ...
    // /tmp and a Docker volume can be on different file systems.
...
async function persistTempFile(file, folder = "") {
  await validateMagicBytes(file);

  const safeFolder = normalizeFolder(folder);
  const directory = await ensureUploadDir(safeFolder);
  const filename = `${crypto.randomUUID()}${extensionFor(file.mimetype)}`;
  const storageKey = safeFolder ? `${safeFolder}/${filename}` : filename;
