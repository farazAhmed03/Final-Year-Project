...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const endAt = new Date(startsAt.getTime() + req.body.durationMinutes * 60 * 1000);
    const conflictStart = new Date(startsAt.getTime() - 90 * 60 * 1000);
    const conflictEnd = new Date(endAt.getTime() + 90 * 60 * 1000);

    const conflict = await Appointment.exists({
      lawyer: lawyer._id,
      status: { $in: [APPOINTMENT_STATUS.REQUESTED, APPOINTMENT_STATUS.CONFIRMED] },
      startsAt: { $gte: conflictStart, $lte: conflictEnd }
    });
    if (conflict) {
      throw new ApiError(409, "This time is too close to another appointment", "APPOINTMENT_CONFLICT");
    }

    const hourlyRate = lawyer.lawyerProfile?.hourlyRate || 0;
    const fee = Math.round((hourlyRate * req.body.durationMinutes / 60) * 100) / 100;
    ...
    const populated = await appointment.populate([
      ...
      { path: "lawyer", select: "name avatarUrl lawyerProfile.specialization" }
    ]);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const roleField = req.user.role === ROLES.CLIENT ? "client" : req.user.role === ROLES.LAWYER ? "lawyer" : null;
    const filter = roleField ? { [roleField]: req.user._id } : {};
...
router.patch(
  "/:id/status",
  validate(Joi.object({
    status: Joi.string().valid(...Object.values(APPOINTMENT_STATUS)).required(),
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!isClient && !isLawyer && !isAdmin) {
      throw new ApiError(403, "You cannot update this appointment", "FORBIDDEN");
    }

    const nextStatus = req.body.status;
    if (!canTransition(appointmentTransitions, appointment.status, nextStatus)) {
      throw new ApiError(409, `Cannot change ${appointment.status} to ${nextStatus}`, "INVALID_STATUS_TRANSITION");
    }

    if (isClient && nextStatus !== APPOINTMENT_STATUS.CANCELLED) {
      throw new ApiError(403, "Clients may only cancel appointments", "FORBIDDEN");
    }
    if (isLawyer && ![
      ...
      throw new ApiError(403, "Lawyer cannot apply this status", "FORBIDDEN");
    }
    if (nextStatus === APPOINTMENT_STATUS.COMPLETED && appointment.startsAt > new Date() && !isAdmin) {
      throw new ApiError(409, "A future appointment cannot be completed", "APPOINTMENT_IN_FUTURE");
    }

    appointment.status = nextStatus;
    appointment.decisionNote = cleanText(req.body.note, 500);
    if (nextStatus === APPOINTMENT_STATUS.CANCELLED) appointment.cancelledBy = req.user._id;
    await appointment.save();

    const recipient = isClient ? appointment.lawyer : appointment.client;
    await createNotification(req.io, {
      user: recipient,
      type: "appointment",
