...
function safeError(error) {
  return {
    ...
    error: {
      ...
      message: error.statusCode && error.statusCode < 500 ? error.message : "Request failed"
    }
  };
}
...
function initializeSockets(io) {
  io.use(async (socket, next) => {
    try {
      ...
      const token = cookies[accessCookie] || socket.handshake.auth?.token;
  ...
  io.on("connection", (socket) => {
    ...
    const expiresIn = Math.max(0, socket.data.tokenExpiresAt - Date.now());
    const expiryTimer = setTimeout(() => socket.disconnect(true), expiresIn);
    socket.on("disconnect", () => clearTimeout(expiryTimer));

    socket.on("conversation:join", async ({ conversationId } = {}, callback = () => {}) => {
      try {
        await assertParticipant(conversationId, user._id);
    ...
    socket.on("typing:start", async ({ conversationId } = {}) => {
      ...
      try {
        ...
        // Typing indicators are best-effort and never affect message delivery.
      }
    });
